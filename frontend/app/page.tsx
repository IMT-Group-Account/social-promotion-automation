...
export default function AdminPage(){
  const [tab,setTab]=useState<'compose'|'calendar'|'accounts'|'analytics'|'operations'>('compose');
  ...
  async function upload(file:File){
    if(content.media.length>=4)throw new Error('미디어는 최대 4개까지 첨부할 수 있습니다.');
    ...
    try{
      ...
      change({...content,media:[...content.media,{type:media.type,url:media.url}]});setNotice(`파일 업로드를 완료했습니다. 현재 ${content.media.length+1}개입니다.`);
  ...
  return <main className="shell">
    ...
    <nav className="tabs" aria-label="홍보 관리 메뉴">{([['compose','작성·검수'],['calendar','예약 캘린더'],['accounts','계정 관리'],['analytics','성과 분석'],['operations','운영 기록']] as const).map(([id,label])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}>{label}</button>)}<button className="new" onClick={reset} disabled={busy}>+ 새 게시물</button></nav>
    ...
    {tab==='compose'&&<div className="layout"><section className="card composer"><div className="card-heading"><h2>{current?immutable?'게시물 확인':'초안 편집':'새 게시물 작성'}</h2><span className="badge">{current?statuses[current.post.status]: '저장 전'}</span></div>
      ...
      <div className="field-title">이미지·동영상 <small>JPEG·PNG·MP4, 파일당 4MB 미만, 최대 4개 / 복수 첨부는 X만 지원</small></div><div className="actions"><label className="button upload">파일 업로드<input type="file" accept="image/jpeg,image/png,video/mp4" onChange={e=>{void choose(e.target.files?.[0],false);e.target.value='';}}/></label><label className="button upload">정사각형으로 잘라 업로드<input type="file" accept="image/jpeg,image/png" onChange={e=>{void choose(e.target.files?.[0],true);e.target.value='';}}/></label></div>
      {uploadProgress!==null&&<label>업로드 {uploadProgress}%<progress value={uploadProgress} max={100}/></label>}
      {content.media.map((m,index)=><div key={`${m.url}-${index}`}>{m.type==='image'?<Image className="preview" src={m.url} alt={`게시할 이미지 ${index+1}`} width={1080} height={1080} unoptimized/>:<video className="preview" src={m.url} controls preload="metadata"/>}<button onClick={()=>change({...content,media:content.media.filter((_,itemIndex)=>itemIndex!==index)})}>파일 {index+1} 제거</button></div>)}
      <details className="external-media"><summary>이미 업로드된 파일 URL 1개로 목록 교체</summary><label>미디어 형식<select value={content.media[0]?.type??'image'} onChange={e=>change({...content,media:content.media.length?[{...content.media[0],type:e.target.value as Media['type']}]:[]})}><option value="image">이미지</option><option value="video">동영상</option></select></label><label>공개 HTTPS 파일 URL<input type="url" value={content.media[0]?.url??''} onChange={e=>change({...content,media:e.target.value?[{type:content.media[0]?.type??'image',url:e.target.value}]:[]})}/></label></details>
    ...
    {tab==='analytics'&&<AnalyticsPanel campaigns={campaigns} connected={connected}/>}
    {tab==='operations'&&<OperationsPanel connected={connected}/>}
  </main>;
}
